  ...
  useEffect(() => {
    ...
    dataVar = dataVar.filter((index) => {
      return index.subject
        .toLowerCase()
        .includes(saveApiInContext.search.toLowerCase());
    });
  ...
  const dataFun = () => {
    const returnData = (stateName) => {
      return stateName.map((index) => (
        <div
          className={props.colCard ?? "col-xxl-4 col-md-6 col-sm-12"}
          key={index.id}
        >
          <div className="bg-light p-4 rounded-3 shadow-custom task">
            <div className="mb-2">
              <div className="d-flex justify-content-between">
                <h4 className="fw-bold">{index.subject}</h4>
                  ...
                  <div className="ms-3">
                    {
                      // play task
                      index.startTask && (
                        <div className="start-task" key={index.id}></div>
                      )
                    }
                  </div>
                </div>
            ...
            <p
              className={
                (props.classScroll_p == false ? "" : "scroll-p") +
                " scroll-custom lh-lg"
              }
            >
              {index.description}
            ...
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <ModalCustom
                  titleModal={"ویرایش تسک"}
                  body={"input"}
                  id={index.id}
                  subjectTask={index.subject}
                  typeTask={index.typeTask}
                  descriptionTask={index.description}
                  roleTask={index.role}
                  dataBsTarget={"modalFormEdit" + index.id}
                />
              </div>
              {!index.startTask && !index.endTask ? (
                <StartTask id={index.id} />
              ) : !index.endTask ? (
                <p>تسک در حال اجرا است.</p>
              ) : (
                <p>تسک به پایان رسیده</p>
              )}
            </div>
          </div>
        </div>
